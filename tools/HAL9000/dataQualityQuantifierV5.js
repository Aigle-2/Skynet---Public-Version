//$ node dataQualityQuantifierV5.js
...
function IdenticalErrorCondition(entry) {
  return (
    !_.isEqual(
      ...
      entry.closerRun.website.currentUrl.protocol
    ) ||
    !_.isEqual(
      entry.failRun.website.currentUrl.host,
      entry.closerRun.website.currentUrl.host
    ) ||
    !_.isEqual(
      entry.failRun.website.currentStatus,
      entry.closerRun.website.currentStatus
    )
  );
}

function DifferentErrorCondition(entry) {
  return _.isEqual(entry.failRun, entry.closerRun);
}
